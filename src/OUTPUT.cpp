...
void *update(void *null)
{
	...
	while(!(p.keymap & QUIT))
	{
		...
		else
		{
			...
			{
				double diff_x = abs(p.pos_x - (double)(int)p.pos_x), diff_y = abs(p.pos_x - (double)(int)p.pos_y);
				if(diff_x < diff_y)
					p.pos_y = late_pos_y;
				else
					p.pos_x = late_pos_x;
		...
		for(int x = 0; x < col; x++)
		{
			...
			while(map[crr_y][crr_x] != '#' && map[crr_y][crr_x] != 'A' && map[crr_y][crr_x] != 'B' && map[crr_y][crr_x] != 'C')
			{
				distance += 0.1d;
